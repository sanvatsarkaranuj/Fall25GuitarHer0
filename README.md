...
vhdl-- SW = '1' (ON): Song mode - use song notes
...
vhdl-- Song start: directly pass button when in song mode (SW=1)
...
vhdl-- Note column 1: Green (leftmost) - position 160
...
Show Image
Key Constants
vhdlCONSTANT NOTE_RADIUS : INTEGER := 18;         -- Radius of circular note
...
vhdlSIGNAL note_col : STD_LOGIC_VECTOR(599 DOWNTO 0) := (OTHERS => '0');
...
vhdlndraw : PROCESS (pixel_row_int, pixel_col_int, horiz_int, note_col)
...
vhdl-- "Ode to Joy" adapted for 4 notes
CONSTANT SONG_COL1 : STD_LOGIC_VECTOR(255 DOWNTO 0) := 
    X"00000000000000008080000000008080808000000000808080800000000080FF";
...
Timing Constants
vhdlCONSTANT TEMPO_BITS : INTEGER := 23;   -- 2^23 / 100MHz = ~84ms per beat
...
vhdl-- Note outputs: output latched values while pulse is active
...
vhdl-- Formula: divider = 100MHz / (2 * desired_frequency)
CONSTANT TONE_GREEN  : INTEGER := 190839;  -- C4 (262 Hz)
CONSTANT TONE_RED    : INTEGER := 151515;  -- E4 (330 Hz)
CONSTANT TONE_PURPLE : INTEGER := 127551;  -- G4 (392 Hz)
CONSTANT TONE_BLUE   : INTEGER := 95602;   -- C5 (523 Hz)
...
Count clock cycles up to the divider value
Toggle the output pin
Repeat for the tone duration (150ms)
The toggling at specific frequencies creates audible tones

Hit Detection (Edge Triggered)
vhdl-- Check for new hits (rising edge detection)
...
Hit Zone Definition
vhdlCONSTANT ZERO_VECTOR : STD_LOGIC_VECTOR(580 downto 530) := (OTHERS => '0');
The hit zone spans rows 530-580 (50 pixels ≈ 130ms timing window).
Hit Detection Logic
vhdlhit_tracker : process(clk)
begin
    if rising_edge(clk) then
...
Detect rising edge of keypress (prevents holding key)
Check if ANY bit in rows 530-580 is '1' (note present)
If yes, increment score by 256
Send hit signal to clear the note
Start timeout to prevent double-hits

Timeout Counter
vhdl-- Timeout counter to prevent double-hits
...
vhdlCONSTANT HEADER_HEIGHT : INTEGER := 40;
...
Flame Animation
vhdl-- Animation counter cycles through 8 phases
IF anim_counter < 5000000 THEN
    anim_counter <= anim_counter + 1;
ELSE
    anim_counter <= 0;
...
Render Priority System
vhdl-- === RENDER PRIORITY (highest to lowest) ===
