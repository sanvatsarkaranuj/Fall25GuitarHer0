...
When 
`SW = '1'`
 (switch up) AND the song is actively playing, notes come from 
`songPlayer`
. Otherwise, notes come from button presses.
####
 Song Start Logic
...
!
[
Note Column Architecture
](
images/block_diagram_notecolumn.png
)
...
`note_col(599 DOWNTO 1) <= note_col(598 DOWNTO 0)`
 shifts all bits down
-
 
`note_col(0) <= note_input`
 adds new note at top
-
...
This was our biggest bug fix. Notes must be held long enough for 
`noteColumn`
 to sample them:
```
vhdl
...
**
 The 
`noteColumn`
 samples its input every 2^18 cycles (2.6ms). By holding the note for 5.2ms, we guarantee at least one sample.
####
 State Machine
